...
interface TemplateFormProps {
  ...
  template?: Template;
  /** Initial type for create mode. Ignored when `template` is provided. */
  defaultType?: TemplateType;
  /** Called after a successful create or update. */
  onSaved?: (template: Template) => void;
  /** When provided, a Cancel button is shown that calls this handler. */
  onCancel?: () => void;
  /**
   * Whether the form is rendered inside a sheet (sticky bar uses popover bg)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {showSettings && form.type === "email" && (
        <section className="space-y-3 rounded-md border border-border bg-muted/20 p-3">
          ...
          <Field label="CC (optional)" error={err("cc")} hint="Comma-separated emails.">
            <Input value={form.cc} onChange={(e) => update("cc", e.target.value)} />
          </Field>
          <Field
            label="BCC (optional)"
            error={err("bcc")}
      ...
      {showSettings && form.type === "notification" && (
        <section className="space-y-3 rounded-md border border-border bg-muted/20 p-3">
          ...
          <Field label="Title" error={err("title")} required>
            ...
            />
            <p className="mt-1 text-xs text-muted-foreground">{form.title.length}/80</p>
          </Field>
          <div className="grid grid-cols-2 gap-3">
      ...
      {showContent && form.type === "email" && (
        <section className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Used by mail clients that don&apos;t render HTML. Auto-derived from the body
            if left blank.
          </p>
          <textarea
      ...
      <div
        className={cn(
          "sticky bottom-0 -mx-4 mt-2 flex flex-wrap items-center justify-end gap-2 border-t border-border px-4 py-3",
          surface === "sheet" ? "bg-popover" : "bg-background"
        )}
      >
        {onCancel ? (
